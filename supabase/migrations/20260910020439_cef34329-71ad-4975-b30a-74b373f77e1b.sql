...
AS $$
  SELECT EXISTS (
    ...
    WHERE user_id = _user_id
      AND fomento_role IN ('superadmin', 'admin', 'gestor', 'auditor', 'coordenador')
  )
$$;
...
BEGIN
  IF NEW.fomento_role IS NOT NULL 
     AND NEW.fomento_role NOT IN ('superadmin', 'admin', 'gestor', 'auditor', 'coordenador') THEN
    RAISE EXCEPTION 'fomento_role must be superadmin, admin, gestor, auditor, coordenador, or NULL';
  END IF;
  RETURN NEW;
END;
$function$;
...
BEGIN
  ...
  END IF;
  IF _role IS NOT NULL AND _role NOT IN ('superadmin', 'admin', 'gestor', 'auditor', 'coordenador') THEN
    RAISE EXCEPTION 'Role must be superadmin, admin, gestor, auditor, coordenador, or NULL';
  END IF;
