...
BEGIN
  ...
  END IF;
  IF NEW.area IS NOT NULL AND NEW.area NOT IN ('pesquisa','inovacao','extensao','ensino','servicos','estagio_tecnico','participacao_evento') THEN
    RAISE EXCEPTION 'area must be pesquisa, inovacao, extensao, ensino, servicos, estagio_tecnico, participacao_evento, or NULL';
  END IF;
